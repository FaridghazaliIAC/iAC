sap.ui.define([
	...
	return Controller.extend("hcm.claim.controller.Main", {
		...
		onAddRow: function() {
			...
			this.getView().byId("idonSubmit").setEnabled(true);
		},
...
// ------------------------------
			// var that = this;
			// var items = this.getView().getModel('local').getData().To_Items;
			// // this.changedStatusItems = [];
			// this.getView().setBusy(true);
			// items.forEach(function(item) {
			// 	var timeDeff = item.idET -item.idST;
...
// ------------------------------
		...
		onSave: function() {
			...
			if (header.Claimid) { // Claim already saved before
		...
		onUpsert: function(header) {
			...
			var mParameters = {
				...
				success: function(odata, resp) {
					...
					that.getView().byId("idonSubmit").setEnabled(true);
					that.getView().setBusy(false);
					MessageToast.show("Claim has been submit.");
